...
#define _IMAGE_MIC_QUXIAO [UIImage imageNamed:@"mic_quxiao.png"]
#define _IMAGE_MIC_WAVE [UIImage imageNamed:@"wave.png"]
...
-(void) createMainUI{
    ...
    CGRect frame2 = CGRectMake(_DEVICE_WIDTH/2-164/2, 200, 136, 136);
    CGRect frame3 = CGRectMake(164/2-50/2, 164/2-93.5/2, 136, 136);
    CGRect frame4 = CGRectMake(80, 50, 43, 41.5);
    ...
    [_talkingImageView addSubview:_dynamicProgress];
    ...
    //_dynamicProgress.center = CGPointMake(110, self.center.y-10);
}
...
-(void) setProgress:(float)progress{
    ...
    if (_talkingImageView.image==_IMAGE_MIC_TALKING) {
        if (_dynamicProgress){
            _dynamicProgress.hidden = NO;
            _dynamicProgress.progress = progress*5;
            if (progress <= 0.01){
...
-(void)showExitViews:(BOOL)yes{
    if (yes) {
        _dynamicProgress.hidden = YES;
        _talkingImageView.image = _IMAGE_MIC_QUXIAO;
    }else{
        _dynamicProgress.hidden = NO;
        _talkingImageView.image = _IMAGE_MIC_TALKING;
    }
