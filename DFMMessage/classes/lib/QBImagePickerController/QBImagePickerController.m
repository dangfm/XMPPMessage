...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self initNavigationWithTitle:self.title];
    
}


#pragma mark 初始化导航
-(void)initNavigationWithTitle:(NSString*)title{
    self.navigationController.navigationBar.backgroundColor = kNavigationBackgroundColor;
    self.navigationController.navigationBar.tintColor = kButtonColor;
    CGFloat w = self.view.frame.size.width;
    CGFloat h = 44;
    if (kSystemVersion>=7) {
        h += 22;
        self.navigationController.navigationBar.barTintColor = kNavigationBackgroundColor;
    }
    [self.navigationController.navigationBar setBackgroundImage:[CommonOperation imageWithColor:kNavigationBackgroundColor andSize:CGSizeMake(w, h)] forBarMetrics:UIBarMetricsDefault];
